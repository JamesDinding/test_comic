...
export function create(opts = {}) {
  if (!opts.wasmURL && !opts.wasmData) {
    return Promise.reject(
      new Error("Either wasmURL or wasmData shall be provided")
    );
  }
  const runtime = getRuntime();
  const imports = { env: runtime };
  return fetchAndInstantiate(opts.wasmData, opts.wasmURL, imports).then(
    (wasm) => {
...
class Dav1d {
  ...
  _decodeFrame(obu, format, unsafe) {
    ...
    const frameRef = this.FFI.djs_decode_obu(
      this.ref,
      obuRef,
      obu.byteLength,
      format
